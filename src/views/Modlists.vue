<template>
  <b-container fluid key="profiles">
      ...
      <b-row align-h="between" align-v="end" class="mb-2">
        <b-col class="pb-0">
          <b-form-select v-model="sort" id="sortBy" @change="sortBy(sort)">
            <b-form-select-option value="modified"
              >Modified ↓</b-form-select-option
            >
            <b-form-select-option value="modified-reverse"
              >Modified ↑</b-form-select-option
            >
            <b-form-select-option value="alpha"
              >Alphabetical ↓</b-form-select-option
            >
            <b-form-select-option value="alpha-reverse"
              >Alphabetical ↑</b-form-select-option
            >
            <b-form-select-option value="created"
              >Created ↓</b-form-select-option
            >
            <b-form-select-option value="created-reverse"
              >Created ↑</b-form-select-option
            >
          </b-form-select>
        </b-col>
        ...
        <b-col align-h="end" class="pb-0">
          <b-button-group>
            <b-button variant="success" @click="showModal('add-modlist-modal')">
              Add new Modlist
            </b-button>
            <b-button variant="info" @click="refreshModlists()">
              Refresh
            </b-button>
          </b-button-group>
        </b-col>
    ...
    <b-modal ref="add-modlist-modal" title="Name Your Profile" hide-footer>
      <b-form @submit="createModlistProfile">
        ...
        <b-form-input
          required
          placeholder="Enter Modlist name"
          v-model="addModal.name"
        />
        <b-input-group>
          <b-form-input
            required
            placeholder="Enter Modlist folder"
    ...
    <b-modal ref="morrowind-warning" title="WARNING" ok-only @ok="reload()">
      <p>
    ...
    >
      <b-form @submit="changeModlist">
        <b-form-input
          v-if="
            (editModal.property != 'game') &
            (editModal.property != 'selectedProfile') &
            (editModal.property != 'exe')
          "
  ...
  data() {
    return {
      ...
      gamesList: [
        ...
        {
          value: 'Skyrim',
          text: 'The Elder Scrolls V: Skyrim Legendary Edition',
        },
        {
  ...
  methods: {
    ...
    createModlistProfile() {
      ...
      try {
        ...
        window.ipcRenderer
          .invoke('create-modlist-profile', { modlistInfo })
          .then((result) => {
            if (result === 'ERROR') {
              this.loading = false
              return
        ...
        this.sendError(
          'F04-03-00',
          'Error while requesting modlist creation!',
          err,
          0
        )
      }
    ...
    openModlistProfile(name) {
      ...
      try {
        ...
        this.sendError(
          'F04-04-00',
          'Error while opening modlist profile folder!',
          err,
          0
        )
      }
    ...
    deleteModlistProfile() {
      ...
      try {
        ...
        this.sendError(
          'F04-05-00',
          'Error while requesting modlist deletion!',
          err,
          0
        )
      }
      ...
      try {
        ...
        if (args[1] === 'selectedProfile') {
          this.profiles
            ...
            .profiles[0].forEach((x) => this.editModal.profiles.push(x))
        }
        if (args[1] === 'exe') {
          this.profiles
            .find((x) => x.name === args[0])
            .executables.forEach((x) => this.editModal.executables.push(x))
        }
        this.showModal('edit-modlist-modal')
      } catch (err) {
        this.sendError(
          'F04-06-00',
          'Error while collecting modlist edit info!',
          err,
          0
        )
      }
    ...
    changeModlist() {
      ...
      try {
        ...
        this.getConfig().then((result) => {
          ...
          result.Modlists[this.editModal.list][
            this.editModal.property
        ...
        this.sendError(
          'F04-07-00',
          'Error while requesting modlist edit!',
          err,
          0
        )
      }
    ...
    launchGame(list) {
      ...
      try {
        ...
        const game = this.profiles[
          this.profiles.findIndex((x) => x.name === list)
        ].game
        if (
          this.currentConfig.Options.gameDirectories.find(
            (x) => x.game === game
          ).path === ''
        ) {
          window.ipcRenderer.send('error', {
            code: 'F04-08-01',
            message:
              'Your game directory for ' +
              game +
              ' could not be found. Please make sure it is set in the options menu and try again',
        ...
        this.sendError(
          'F04-08-00',
          'Error while initiating game launch!',
          err,
          0
        )
      }
    ...
    launchMO2(list) {
      window.ipcRenderer.invoke('launch-mo2', { listName: list })
    },
    refreshModlists() {
      this.loading = true
      ...
      window.ipcRenderer.invoke('refresh-modlists').then((result) => {
        this.getConfig().then((result) => {
          ...
          Object.entries(this.currentConfig.Modlists).forEach((key) => {
            ...
            this.profiles[
              Object.keys(this.currentConfig.Modlists).indexOf(key[1].name)
            ].profiles.push([
