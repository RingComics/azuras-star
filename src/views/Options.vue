  ...
  <div>
    <b-overlay :show="loading">
      ...
      <b-form @submit="saveConfig()">
        ...
        <b-collapse id="directories">
          <b-form-group>
            ...
            <div v-for="(gameDirectory, index) in gameDirectories" :key="index">
              <span> {{ gameDirectory.game }} Directory </span><br />
              <b-input-group>
                <b-form-input
                ...
                <b-input-group-append>
                  <b-button
                    v-b-popover.hover.top="
                      'Change ' + gameDirectory.game + ' Directory'
                    "
        ...
        <b-button @click="showModal('confirm-config-reset')" variant="danger">
          Reset configurations
        </b-button>
        <br />
        <b-checkbox v-model="advancedOptions" @change="saveConfig()">
          <span
            v-b-popover.hover.top="
              'Enables extra settings, modlist options, and the developer page (coming soon)'
            "
          >
            Advanced Options
          </span>
        ...
        <b-card-header header-tag="header" class="p-1" role="tab">
          <b-button block v-b-toggle.config v-if="advancedOptions">
        ...
        <b-collapse id="config">
          <b-button
            ...
            @click="openConfig()"
            >Open config file</b-button
          >
          <ul>
            <li>
              <strong><u>Version:</u></strong> {{ currentConfig.version }}
            </li>
            <li>
              <strong><u>App Path:</u></strong> {{ currentConfig.ASPath }}
            </li>
            <li>
              <strong><u>Development:</u></strong>
            ...
            <ul>
              <li>
                ...
                {{ currentConfig.Options.advancedOptions }}
              </li>
              <li>
                <strong><u>Game Directories</u></strong>
              </li>
              <ul>
                <div
                  v-for="(entry, index) in currentConfig.Options
                    .gameDirectories"
                  :key="index"
                >
                  <li>
                    <strong
                      ><u>{{ entry.game }}:</u></strong
                    >
                    {{ entry.path }}
                  </li>
                </div>
            ...
            <ul>
              ...
              >
                ...
                <ul>
                  <div
                    v-for="(entry, index2) in Object.keys(
                      currentConfig.Modlists[list]
                    )"
                    :key="index2"
                  >
                    <li
                      v-if="
                        typeof currentConfig.Modlists[list][entry] == 'string'
                      "
                    >
                      <strong
                        ><u>{{ entry }}:</u></strong
                      >
                      {{ currentConfig.Modlists[list][entry] }}
                    </li>
                    <div
                    ...
                    >
                      ...
                      <ul>
                        <li
                          v-for="(value, index3) in currentConfig.Modlists[
                            list
                          ][entry]"
                          :key="index3"
                        >
                    ...
                    <li
                      v-if="
                        typeof currentConfig.Modlists[list][entry] == 'number'
                      "
                    >
                      <strong
                        ><u>{{ entry }}:</u></strong
                      >
                      {{ Date(currentConfig.Modlists[list][entry]) }}
                    </li>
                  </div>
                </ul>
              </div>
    ...
    <!--Modals-->
    <b-modal ref="confirm-config-reset" title="Reset settings?" hide-footer>
      <p>
        This will clear all of your modlists and settings (Your modlists will
        not be deleted from your computer)
      </p>
      <b-button variant="danger" @click="resetSettings()"> Yes </b-button>
      <b-button @click="hideModal('confirm-config-reset')"> No </b-button>
    </b-modal>
  </div>
</template>
  ...
  methods: {
    ...
    saveConfig() {
      ...
      try {
        ...
        window.ipcRenderer.send('update-config', {
