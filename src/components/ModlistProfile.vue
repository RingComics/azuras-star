    ...
    <b-collapse :id="'accordion-'+this.$props.index" role="tabpanel" accordion="modlist">
        <b-tabs>
          <b-tab title="Play" active>
            <b-button v-b-popover.hover.top="'Change the Mod Organizer profile used when launching the game'" variant="primary" v-if="$props.profiles.length > 1" @click="$emit('change-modlist', [ name, 'selectedProfile', $props.profiles])">Change Profile</b-button>
            <br/>
          ...
          <b-tab title="Options">
            <p class="text-left">Name: {{ this.$props.name }} <b-button v-b-popover.hover.top="'Change modlist name'" variant="primary" @click="$emit('change-modlist', [ name, 'name'])" class="float-right">Change</b-button></p>
            <p class="text-left">Path: {{ this.$props.path }} <b-button v-b-popover.hover.top="'Change modlist path'" variant="primary" @click="$emit('change-modlist', [ name, 'path'])" class="float-right">Change</b-button></p>
