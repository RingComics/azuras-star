...
export function refreshModlists() {
  try {
    ...
    Object.keys(config.Modlists).forEach((list) => {
      ...
      const MO2ini = ini.parse(
        fs.readFileSync(
          path.join(config.Modlists[list].path, 'ModOrganizer.ini'),
          'utf-8'
        )
      )
      const modlistInfo = {
      ...
      Object.keys(MO2ini.customExecutables).forEach((entry) => {
        if (
          ...
          MO2ini.customExecutables[entry] != undefined
        ) {
      ...
      // Time
      if (config.Modlists[list].created === undefined)
        config.Modlists[modlistInfo.name].created = Date.now()
      if (config.Modlists[list].modified === undefined)
        config.Modlists[modlistInfo.name].modified = Date.now()
    })
...
export function createModlist(modlistInfo) {
  try {
    ...
    toLog(
      'Creating new Modlist, ' + modlistInfo.name + '\n' + '='.repeat(80),
      0
    )
    ...
    const MO2ini = ini.parse(
      fs.readFileSync(path.join(modlistInfo.path, 'ModOrganizer.ini'), 'utf-8')
    )
    ...
    switch (modlistInfo.game) {
      ...
      default:
        sendError(
          'B06-07-03',
          modlistInfo.game +
            ' is not supported! If you would like to see support added, please comment on the Supported Games pinned issue on GitHub',
...
export function launchGame(list) {
  try {
    ...
    toLog('Game: ' + game, 2)
    const gamePath = currentConfig.Options.gameDirectories.find(
      (x) => x.game === game
    ).path
    ...
    const isGameRunning = setInterval(checkProcess, 1000)
    /**
