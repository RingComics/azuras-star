...
export const URI = isDevelopment
  ? 'https://testing.loadorderlibrary.com/'
  : 'https://www.loadorderlibrary.com/'
  ...
  try {
    ...
    request(options, (err, res, body) => {
      if (err) {
        sendError(
          'B04-02-01',
          'Error while sending Load Order Library request',
          err
        )
        return 'ERROR'
      }
      switch (res.statusCode) {
        ...
        case 404:
          sendError(
            'B04-02-02',
            'LoadOrderLibrary not available!',
    ...
    sendError(
      'B04-02-00',
      'Error while checking Load Order Library availability',
      err
    )
    return 'ERROR'
  }
    ...
    try {
      ...
      fs.readdir(list.path, (err, files) => {
        if (err) {
          sendError(
            'B04-02-01:01',
            'Error while reading profile directory',
            err
          )
          return 'ERROR'
        }
        files.forEach((file) => {
          ...
          if (
            (path.extname(path.join(list.path, file)) == 'txt' ||
              path.extname(path.join(list.name, file)) == 'ini') &&
            fs.statSync(path.join(list.name, file)).size <= 128000
          ) {
            form['files[]'].push(
              fs.createReadStream(path.join(list.name, file))
            )
          }
        })
      })
