...
function outputConsole(out) {
  let date = new Date();
  let timestamp =
    date.getHours() + ":" + date.getMinutes() + ":" + date.getSeconds() + " > ";
  document.getElementById("user-console").value += timestamp + out + "\n";
}
    ...
    if (modpackInfo["Vanilla (Legendary Edition)"].path == "") {
      Promise.resolve(
        ...
        if (value.DisplayName) {
          ...
          updateModpackListJson(
            "Vanilla (Legendary Edition)",
            "path",
            value.InstallLocation + "TESV.exe"
          );
        } else {
          document.getElementById("check-le").innerHTML =
